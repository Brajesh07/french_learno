...
export async function GET(
  ...
  try {
    ...
    const { data: quizzes, error: quizzesError } = await supabase
      ...
      .eq("course_id", id).eq("created_by", auth.userId)
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { return writeTeacherContent(request, 'course', (await params).id); }
